...
def evaluate_clustering(dataframe, exclude_cols=['Local_Authority_Code', 'Local_Authority_Name']):
    ...
    # Select numeric columns, excluding any specified
    ...
    # Impute any remaining missing values using KNN
    imputer = KNNImputer(n_neighbors=5)
    X_imputed = imputer.fit_transform(X)
    ...
    # Define clustering algorithms and their hyperparameter grids
    clustering_algorithms = {
        'KMeans': {
            'model': KMeans,
    ...
    # Iterate over different UMAP output dimensions
    ...
    # Compute mean score across all metrics
    results_df['mean_score'] = results_df[[
        'silhouette_score_scaled',
        'calinski_harabasz_scaled',
        'davies_bouldin_scaled'
    ...
    # Sort by Calinski-Harabasz (or change to 'mean_score' as needed)
